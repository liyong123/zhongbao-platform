...
import editorStr from './textEditor.html';
  ...
  render() {
    return (
      <div
        style={{ width: '80%' }}
        ref={dom => {
          if (!dom || this.isEditorInit) return;
          this.isEditorInit = true;
          window._setUe = () => {
            const ue = window.UE.getEditor('container', {
              toolbars: this.props.icons,
            });
            this.ue = ue;
            ue.ready(() => {
              ue.setContent(this.props.value);
              ue.addListener('contentChange', this.onChange);
            });
          };
          $(editorStr).appendTo(dom);
        }}
        id="editor"
      />
    );
